...
export const render = () => {
  ...
  /* ── the order ─────────────────────────────────────────── */
  s += text(890, 146, "The order", { size: 24, caps: true });
  s += dashed(596, 164, 1210, 164);
  ...
  for (const [x, n, title, items, lock] of phases) {
    s += rect(x, 188, 172, 152, { strokeWidth: 2.8 });
    s += circle(x + 26, 214, 30);
    s += text(x + 26, 221, n, { size: 18, weight: 900 });
    s += text(x + 106, 221, title, { size: 22 });
    items.forEach((it, i) => { s += text(x + 86, 258 + i * 22, it, { size: 12, weight: 600 }); });
    const gx = x + 172;
    s += icon.check(gx + 12, 306, 22);
    s += text(gx + 12, 332, lock, { size: 10, caps: true, weight: 900, tracking: 1 });
    if (x < 1012) s += arrow(gx + 2, 258, gx + 22, 258, { head: 12 });
  }
  s += text(900, 378, "Three locks never move. Everything else is a range your market gets a vote on.",
            { size: 15, weight: 700 });

  /* ── the gates ─────────────────────────────────────────── */
  s += dashed(30, 416, 1210, 416);
  s += text(620, 458, "What unlocks, and when", { size: 26, caps: true });
  ...
  for (const [x, n, title, ask, gives] of gates) {
    s += rect(x, 496, 296, 174, { strokeWidth: 2.6 });
    s += circle(x + 34, 530, 34);
    s += text(x + 34, 538, n, { size: 19, weight: 900 });
    s += text(x + 178, 538, title, { size: 19 });
    s += text(x + 148, 578, "30 seconds. You answer", { size: 11, weight: 700, caps: true });
    s += text(x + 148, 604, ask, { size: 14, weight: 700 });
    s += line(x + 22, 620, x + 274, 620, { strokeWidth: 1.6 });
    s += text(x + 148, 642, "You get", { size: 10, caps: true, weight: 900, tracking: 1.2 });
    s += text(x + 148, 662, gives, { size: 14 });
    if (x < 836) s += arrow(x + 302, 582, x + 350, 582, { head: 14 });
  }
